        ...
        <footer className="footer-glass pt-20 pb-10 px-6 lg:px-8">
            <div className="section-shell">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-12 mb-14">
                    ...
                    <div>
                        <h3 className="text-[11px] font-bold text-subtle uppercase tracking-[0.15em] mb-5">İletişim</h3>
                        <ul className="space-y-3 text-sm text-muted">
                            ...
                            <li className="pt-2 flex gap-4">
                                {settings?.socialLinks?.github && <a href={settings.socialLinks.github} target="_blank" rel="noopener noreferrer" className="hover:text-fg transition-colors duration-300">GitHub</a>}
                                {settings?.socialLinks?.linkedin && <a href={settings.socialLinks.linkedin} target="_blank" rel="noopener noreferrer" className="hover:text-fg transition-colors duration-300">LinkedIn</a>}
                                {settings?.socialLinks?.x && <a href={settings.socialLinks.x} target="_blank" rel="noopener noreferrer" className="hover:text-fg transition-colors duration-300">X</a>}
                            </li>
                        </ul>
                    </div>
                </div>
                <div className="flex flex-col md:flex-row justify-between items-center text-xs text-subtle pt-8 border-t border-card-border">
                    ...
                    <Link href="/kvkk" className="hover:text-fg transition-colors duration-300 mt-2 md:mt-0">KVKK &amp; Gizlilik</Link>
                </div>
            </div>
        </footer>
    )
