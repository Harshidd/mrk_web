...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
    ...
    if (!d) return {}
    return { title: d.title, description: d.shortSummary, alternates: { canonical: `/cozumler/${d.slug.current}` } }
}
...
export default async function CozumlerDetail({ params }: { params: Promise<{ slug: string }> }) {
    const { slug } = await params
            ...
            <article className="section-shell">
                <AnimatedSection className="pb-14 border-b border-card-border mb-14">
                    <div className="flex items-center gap-2 text-sm text-subtle mb-8">
                        <Link href="/cozumler" className="hover:text-fg transition-colors">Çözümler</Link><ChevronRight size={14} /><span className="text-fg">{cat.title}</span>
                    </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-14">
                    <div className="lg:col-span-8">
                        <StaggerChildren className="flex flex-col gap-14">
                            {cat.problemItSolves && <StaggerItem><h2 className="text-2xl font-semibold text-fg mb-5">Çözülen problem</h2><div className="text-muted"><Blocks value={cat.problemItSolves} /></div></StaggerItem>}
                            ...
                            {cat.delivery && <StaggerItem><h2 className="text-2xl font-semibold text-fg mb-5">Teslimat</h2><div className="text-muted"><Blocks value={cat.delivery} /></div></StaggerItem>}
                            {cat.faqs && cat.faqs.length > 0 && (
                                <StaggerItem>
                                    <h2 className="text-2xl font-semibold text-fg mb-6 pt-6 border-t border-card-border">SSS</h2>
                    ...
                    <AnimatedSection delay={0.2} className="lg:col-span-4">
                        <div className="sticky top-32 glass p-8 text-center">
                            <h3 className="text-xl font-semibold text-fg mb-4">İletişime geçin</h3>
                            <p className="text-sm text-subtle mb-7">Projenizi detaylandırmak için bize ulaşabilirsiniz.</p>
